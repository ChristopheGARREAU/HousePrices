...
library(car)
vif(reg1)
# Oups des variables aliasées ont été détectés. Pas bien.
# Variables aliasées = variables sensiblement identiques 
# à une addition ou une multiplication près
...
# findCorrelation (ci-dessous) fait partie
# du package caret
correlations.fortes <- findCorrelation(matrice.cor, cutoff = 0.6)
colnames(dataNum)[correlations.fortes]
# On élimine les colonnes correspondantes
# Dans le jeu de données
datareg<-select(datareg, -GrLivArea, -GarageCars,  
                -X1stFlrSF,-GarageYrBlt,-TotRmsAbvGrd)
...
# Tester la significativité des var. explicatives
# catégorielles
library(car)
Anova(reg1)
# Pour voir la direction de l'influence d'une
# var quanti sur la var. à expliquer, 
# Regarder les coefficients du summary.
# Pour visualiser la direction de l'influence
# d'une catégorielle, faites des boxplot. 
...
# Diagnostic
plot(reg1, 1)
# Les observations 826 et 633 
# sont trop turbulentes
# Eventuellement les éliminer 
# et reprendre le calcul

# Autres outils de régression: voir fiches résumé
# page 18
